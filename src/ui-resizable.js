(function (w, d) {
    var init = function (element, options) {
        ...
        if (this.options.borders.n)
            this.border.n = $('<div class="resizable-handle resizable-handle-n-border">')
                .appendTo(this.element)
                .on('mousedown', {direction: 'n'}, this.mousedown.bind(this));

        if (this.options.borders.e)
            this.border.e = $('<div class="resizable-handle resizable-handle-e-border">')
                .appendTo(this.element)
                .on('mousedown', {direction: 'e'}, this.mousedown.bind(this));

        if (this.options.borders.s)
            this.border.s = $('<div class="resizable-handle resizable-handle-s-border">')
                .appendTo(this.element)
                .on('mousedown', {direction: 's'}, this.mousedown.bind(this));

        if (this.options.borders.w)
            this.border.w = $('<div class="resizable-handle resizable-handle-w-border">')
                .appendTo(this.element)
                .on('mousedown', {direction: 'w'}, this.mousedown.bind(this));

        this.handle = {};

        if (this.options.handles.n)
            this.handle.n = $('<div class="resizable-handle resizable-handle-n">')
                .appendTo(this.element)
                .on('mousedown', {direction: 'n'}, this.mousedown.bind(this));

        if (this.options.handles.ne)
            this.handle.ne = $('<div class="resizable-handle resizable-handle-ne">')
                .appendTo(this.element)
                .on('mousedown', {direction: 'ne'}, this.mousedown.bind(this));

        if (this.options.handles.e)
            this.handle.e = $('<div class="resizable-handle resizable-handle-e">')
                .appendTo(this.element)
                .on('mousedown', {direction: 'e'}, this.mousedown.bind(this));

        if (this.options.handles.es)
            this.handle.es = $('<div class="resizable-handle resizable-handle-es">')
                .appendTo(this.element)
                .on('mousedown', {direction: 'es'}, this.mousedown.bind(this));

        if (this.options.handles.s)
            this.handle.s = $('<div class="resizable-handle resizable-handle-s">')
                .appendTo(this.element)
                .on('mousedown', {direction: 's'}, this.mousedown.bind(this));

        if (this.options.handles.sw)
            this.handle.sw = $('<div class="resizable-handle resizable-handle-sw">')
                .appendTo(this.element)
                .on('mousedown', {direction: 'sw'}, this.mousedown.bind(this));

        if (this.options.handles.w)
            this.handle.w = $('<div class="resizable-handle resizable-handle-w">')
                .appendTo(this.element)
                .on('mousedown', {direction: 'w'}, this.mousedown.bind(this));

        if (this.options.handles.nw)
            this.handle.nw = $('<div class="resizable-handle resizable-handle-nw">')
                .appendTo(this.element)
                .on('mousedown', {direction: 'nw'}, this.mousedown.bind(this));
    ...
    init.prototype = {
        mousedown: function (e) {
            ...
            $(d)
                ...
                .one('mouseup', $.proxy(this.mouseup, this));
        ...
        mousemove: function (e) {
            ...
            if (this.options.resizingX && (this.direction == 'e' || this.direction == 'w' || this.direction == 'ne' || this.direction == 'es' || this.direction == 'sw' || this.direction == 'nw')) {
                ...
                if (this.current.width < this.options.minWidth) this.current.width = this.options.minWidth;
                else if (this.options.maxWidth && this.current.width > this.options.maxWidth) this.current.width = this.options.maxWidth;
            }
            ...
            if (this.options.resizingY && (this.direction == 'n' || this.direction == 's' || this.direction == 'ne' || this.direction == 'es' || this.direction == 'sw' || this.direction == 'nw')) {
                ...
                if (this.current.height < this.options.minHeight) this.current.height = this.options.minHeight;
                else if (this.options.maxHeight && this.current.height > this.options.maxHeight) this.current.width = this.options.maxHeight;
            }
            ...
            switch (this.direction) {
                case 'n':
                    if (this.options.position)
                        this.movable.css('top', this.current.top + 'px');
                    else
                        this.movable.css('margin-top', this.current.top + 'px');
                ...
                case 'sw':
                    if (this.options.position)
                        this.movable.css('left', this.current.left + 'px');
                    else
                        this.movable.css('margin-left', this.current.left + 'px');
