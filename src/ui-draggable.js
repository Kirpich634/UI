(function (w, d) {
    ...
    init.prototype = {
        setDraggable: function (draggable) {
            ...
            if ( is.string(this.options.draggable) )
                this.draggable = this.element.find(this.options.draggable);
            else if ( is.$(this.options.draggable) )
                this.draggable = this.options.draggable;
            else if ( is.HTMLElement(this.options.draggable) )
                this.draggable = $(this.options.draggable);
            else
        ...
        setContainment: function (containment) {
            ...
            if ( is.string(this.options.containment) )
                this.containment = this.element.closest(this.options.containment);
            else if ( is.$(this.options.containment) )
                this.containment = this.options.containment;
            else if ( is.HTMLElement(this.options.containment) )
                this.containment = $(this.options.containment);
        },
        ...
        containmentFix: function (position) {
            if (this.options.containmentFor == 'cursor') {

                if (this.offsetParentRect.top + position.top + this.prefix.top < this.containmentRect.top)
                    position.top = this.containmentRect.top - this.offsetParentRect.top - this.prefix.top;
                else if (this.offsetParentRect.top + position.top + this.prefix.top > this.containmentRect.bottom)
                    position.top = this.offsetParentSize.height + this.containmentRect.bottom - this.offsetParentRect.bottom - this.prefix.top;

                if (this.offsetParentRect.left + position.left + this.prefix.left < this.containmentRect.left )
                    position.left = this.containmentRect.left - this.offsetParentRect.left - this.prefix.left;
                else if (this.offsetParentRect.left + position.left + this.prefix.left > this.containmentRect.right )
                    position.left = this.offsetParentSize.width + this.containmentRect.right - this.offsetParentRect.right - this.prefix.left;

            } else {
                if (this.offsetParentRect.top + position.top < this.containmentRect.top )
                    position.top = this.containmentRect.top - this.offsetParentRect.top;
                else if (this.offsetParentRect.top + position.top + this.draggableSize.height > this.containmentRect.bottom )
                    position.top = this.offsetParentSize.height + this.containmentRect.bottom - this.offsetParentRect.bottom - this.draggableSize.height;

                if (this.offsetParentRect.left + position.left < this.containmentRect.left )
                    position.left = this.containmentRect.left - this.offsetParentRect.left;
                else if (this.offsetParentRect.left + position.left + this.draggableSize.width > this.containmentRect.right )
                    position.left = this.offsetParentSize.width + this.containmentRect.right - this.offsetParentRect.right - this.draggableSize.width;
            }
        },
        ...
        axisMove: function (position) {
            requestAnimationFrame(function () {
                if ( this.options.axis == 'x' )
                    this.draggable.css('left', position.left);
                else if (  this.options.axis == 'y' )
                    this.draggable.css('top', position.top);
                else
                    this.draggable.css({top: position.top, left: position.left});
            }.bind(this));
        },
        ...
        mousedown: function (e) {
            ...
            if ( this.draggable.has(this.handle).length == 0  && this.draggable[0] != this.handle[0]) {
                if ( x < rect.left || x > rect.left + this.draggable.outerWidth() )
                    this.prefix.left = this.prefix.right = this.draggable.outerWidth() / 2;

                if ( y < rect.top || y > rect.top + this.draggable.outerHeight() )
                    this.prefix.top = this.prefix.bottom = this.draggable.outerHeight() / 2;
        ...
        move: function (e) {
            ...
            if (this.containment)
                this.containmentFix(this.position);

            this.element.trigger('draggable:drag', [this]);

            this.setLastPosition();

            if ( this.options.preventmove == false )
                this.axisMove(this.position);
        },
            ...
            switch (arguments[0]) {
                case 'preventmove':
                    if (arguments[1] != undefined)
                        draggable.options.preventmove = arguments[1];
                    else
                        return draggable.options.preventmove;

                    break;
                case 'axis':
                    if (arguments[1] != undefined)
                        draggable.options.axis = arguments[1];
                    else
                        return draggable.options.axis;

                    break;
                case 'containment':
                    if (arguments[1] != undefined)
                        draggable.setContainment(arguments[1]);
                    else
                        return draggable.options.containment;

                    break;
                case 'containmentFor':
                    if (arguments[1] != undefined)
                        draggable.options.containmentFor = arguments[1];
                    else
                        return draggable.options.containmentFor;

                    break;
                case 'handle':
                    if (arguments[1] != undefined)
                        draggable.setHandle(arguments[1]);
                    else
                        return draggable.options.handle;

                    break;
                case 'draggable':
                    if (arguments[1] != undefined)
                        draggable.setDraggable(arguments[1]);
                    else
                        return draggable.options.handle;
