(function ($) {
    ...
    var init = function (parent, config) {
    ...
    init.prototype = {
        ...
        slide: function (e, ui) {
            ...
            if ( this.config.orientation == 'vertical' )
                this.value = this.min + (this.max - this.min) * (this.draggable.position.top / (this.draggable.containment.outerHeight() - this.draggable.draggableSize.height));
            else
                this.value = this.min + (this.max - this.min) * (this.draggable.position.left / (this.draggable.containment.outerWidth() - this.draggable.draggableSize.width));
    ...
    $.fn.uislider = function () {
        if (this.data('slider')) {
            ...
            switch (arguments[0]) {
                ...
                case 'value':
                    if (arguments[1] != undefined){
                        if (arguments[1] < slider.min)
                            slider.value = slider.min;
                        else if (arguments[1] > slider.max)
                        ...
                        if ( slider.config.orientation == 'vertical' )
                            slider.draggable.css('transform', 'translateY(' + shift  + 'px)');
                        else
                            slider.draggable.css('transform', 'translateX(' + shift + 'px)');
                    } else
                        return slider.value;
