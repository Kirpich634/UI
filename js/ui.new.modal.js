(function (w, d) {
    ...
    init.prototype = {
        ...
        open: function () {
            ...
            if ( is.nodeList(this.options.content) || is.text(this.options.content) ) {
                ...
                if (this.options.content.length > 1) {
                    ...
                    this.window = this.options.content;
            ...
            if (this.options.substrate)
                this.substrate = $('<div>')
                    .append(this.window)
                    .addClass('modal substrate fade transparent ' + this.options.substrateClass)
                    .on('click', function(e){
                        if (!testClickLocation(e.target, this.window) && this.permission)
                            this.close();
                    }.bind(this));

            if (this.options.closeButton)
                this.closeButton = this.window.find(this.options.closeButton).on('click', function (e) {
                    e.preventDefault();
            ...
            var openEffect;
            if ( is.function(this.options.effect) )
                openEffect = this.options.effect;
            else if ( is.string(this.options.effect) && is.function(effects.open[this.options.effect]) )
                openEffect = effects.open[this.options.effect];
            else if ( is.object(this.options.effect) && is.function(this.options.effect.open) )
                openEffect = this.options.effect.open;
            else if ( is.object(this.options.effect) && is.string(this.options.effect.open) && is.function(effects.open[this.options.effect.open]) )
                openEffect = effects.open[this.options.effect.open];
            else
                openEffect = effects.open['transform'];

            this.element.trigger('modal:open', [this]);

            openEffect(this, function() {
                if (this.substrate)
                    this.substrate.appendTo('body');
                else
        ...
        close: function () {
            ...
            var closeEffect;
            if ( is.function(this.options.effect) )
                closeEffect = this.options.effect;
            else if ( is.string(this.options.effect) && is.function(effects.close[this.options.effect]) )
                closeEffect = effects.close[this.options.effect];
            else if ( is.object(this.options.effect) && is.function(this.options.effect.close) )
                closeEffect = this.options.effect.close;
            else if ( is.object(this.options.effect) && is.string(this.options.effect.close) && is.function(effects.close[this.options.effect.close]) )
                closeEffect = effects.close[this.options.effect.close];
            else
                closeEffect = effects.close['transform'];
    ...
    $.fn.modal = function (options) {
        if ( this.data('modal') ) {
            ...
            if ( is.HTMLTemplateElement(options.content) )
                options.content = $(options.content.content.childNodes);
            else if ( is.documentFragment(options.content) )
                options.content = $(options.content.childNodes);
            else if ( is.$(options.content) && options.content.length == 1 && is.HTMLTemplateElement(options.content[0]) )
                options.content = $(options.content[0].content.childNodes);
            else if ( is.$(options.content) && options.content.length == 1 && is.documentFragment(options.content[0]) )
                options.content = $(options.content[0].childNodes);
