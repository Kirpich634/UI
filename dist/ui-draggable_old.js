    ...
    $.widget('draggable', {
        ...
        setDraggable: function setDraggable(draggable) {
            ...
            if (this.options.draggable) {
                if (is.string(this.options.draggable)) this.draggable = this.parent.find(this.options.draggable);else if (is.$(this.options.draggable)) this.draggable = this.options.draggable;else this.draggable = $(this.options.draggable);
            } else this.draggable = this.parent;
        ...
        setContainment: function setContainment(containment) {
            ...
            if (this.options.containment) {
                if (is.string(this.options.containment)) this.containment = this.parent.closest(this.options.containment);else if (is.$(this.options.handle)) this.containment = this.options.containment;else this.containment = $(this.options.containment);
            }
        },
        setHandle: function setHandle(handle) {
            ...
            if (this.options.handle) {
                if (is.string(this.options.handle)) this.handle = this.parent.find(this.options.handle);else if (is.$(this.options.handle)) this.handle = this.options.handle;else this.handle = $(this.options.handle);
            } else this.handle = this.parent;
        ...
        getPosition: function getPosition() {
            ...
            if (cssPosition == 'fixed') {
                ...
                if (this.containment && this.options.containmentFor == 'cursor') {
                    if (y < containmentOffset.top) position.top = containmentOffset.top - this.cursorOffset.top;else if (y > containmentOffset.top + containmentHeight) position.top = containmentOffset.top + containmentHeight - this.cursorOffset.top;

                    if (x < containmentOffset.left) position.left = containmentOffset.left - this.cursorOffset.left;else if (x > containmentOffset.left + containmentWidth) position.left = containmentOffset.left + containmentWidth - this.cursorOffset.left;
                } else if (this.containment && this.options.containmentFor == 'element') {

                    if (y - this.cursorOffset.top < containmentOffset.top) position.top = containmentOffset.top;else if (y - this.cursorOffset.top + helperHeight > containmentOffset.top + containmentHeight) position.top = containmentOffset.top + containmentHeight - helperHeight;

                    if (x - this.cursorOffset.left < containmentOffset.left) position.left = containmentOffset.left;else if (x - this.cursorOffset.left + helperWidth > containmentOffset.left + containmentWidth) position.left = containmentOffset.left + containmentWidth - helperWidth;
                }
            } else {
                ...
                if (this.containment && this.options.containmentFor == 'cursor') {
                    if (y < containmentOffset.top) position.top = containmentOffset.top - offsetParentOffset.top - this.cursorOffset.top;else if (y > containmentOffset.top + containmentHeight) position.top = containmentOffset.top + containmentHeight - offsetParentOffset.top - this.cursorOffset.top;

                    if (x < containmentOffset.left) position.left = containmentOffset.left - offsetParentOffset.left - this.cursorOffset.left;else if (x > containmentOffset.left + containmentWidth) position.left = containmentOffset.left + containmentWidth - offsetParentOffset.left - this.cursorOffset.left;
                } else if (this.containment && this.options.containmentFor == 'element') {
                    if (y - this.cursorOffset.top < containmentOffset.top) position.top = containmentOffset.top - offsetParentOffset.top;else if (y - this.cursorOffset.top + helperHeight > containmentOffset.top + containmentHeight) position.top = containmentOffset.top - offsetParentOffset.top + containmentHeight - helperHeight;

                    if (x - this.cursorOffset.left < containmentOffset.left) position.left = containmentOffset.left - offsetParentOffset.left;else if (x - this.cursorOffset.left + helperWidth > containmentOffset.left + containmentWidth) position.left = containmentOffset.left - offsetParentOffset.left + containmentWidth - helperWidth;
                }
        ...
        setHelper: function setHelper() {
            if (this.options.helper == 'original') this.helper = this.draggable;else if (this.options.helper == 'clone') {
        ...
        getRelativeFix: function getRelativeFix(draggableOffset) {
            return {
        ...
        setOriginPosition: function setOriginPosition(draggableOffset, draggableWidth, draggableHeight, helperWidth, helperHeight) {
            this.originPosition = {
            ...
            if (draggableWidth != helperWidth) this.originPosition.left += (draggableWidth - helperWidth) / 2;

            if (draggableHeight != helperHeight) this.originPosition.top += (draggableHeight - helperHeight) / 2;
        },

        setCursorOffset: function setCursorOffset(x, y, draggableOffset, draggableWidth, draggableHeight, helperWidth, helperHeight) {
            this.cursorOffset = {
                top: 0,
                left: 0
            };
            ...
            if (this.options.cursorOffset == 'auto') {
                ...
                if (draggableWidth != helperWidth) this.cursorOffset.left -= (draggableWidth - helperWidth) / 2;

                if (draggableHeight != helperHeight) this.cursorOffset.top -= (draggableHeight - helperHeight) / 2;
        ...
        axisMove: function axisMove(position) {
            ...
            if (this.options.axis == 'x') this.helper.css('left', position.left);else if (this.options.axis == 'y') this.helper.css('top', position.top);else this.helper.css({ top: position.top, left: position.left });
        },
        ...
        mousedown: function mousedown(e) {
            ...
            if (this.cssPosition == 'relative') this.relativeFix = this.getRelativeFix(draggableOffset);
        ...
        setOption: function setOption(kay, value) {
            switch (kay) {
                case 'preventmove':
                    if (value != undefined) this.options.preventmove = value;else return this.options.preventmove;

                    break;
                case 'axis':
                    if (value != undefined) this.options.axis = value;else return this.options.axis;

                    break;
                case 'containment':
                    if (value != undefined) this.setContainment(value);else return this.options.containment;

                    break;
                case 'containmentFor':
                    if (value != undefined) this.options.containmentFor = value;else return this.options.containmentFor;

                    break;
                case 'handle':
                    if (value != undefined) this.setHandle(value);else return this.options.handle;

                    break;
                case 'draggable':
                    if (value != undefined) this.setDraggable(value);else return this.options.handle;
